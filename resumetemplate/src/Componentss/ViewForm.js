...
constructor(props){
    ...
    axios.get('http://localhost:8080/fresher' + '/'+this.state.fresherId).then((response)=>{
      console.log(response)
        ...
        this.state.message="View Successfully!!!!";
         }).catch((error)=>{
...
render(){
    return (
      ...
      <div className = "card-body">
       ...
       <p class="text-dark">AGE        :     {this.state.users.age}</p>
       <p class="text-dark">ADDRESS    :      {this.state.users.address}</p>
       <h4 class="text-info"><b>TECHNICAL SKILL</b></h4>
       <p class="text-dark">SKILLNAME   : {this.state.users?.skills?.[0]?.skillName} ,{this.state.users?.skills?.[1]?.skillName}</p>
       ...
       <p class="text-dark">PROJECTNAME   : {this.state.users?.projects?.[0]?.projectName} ,{this.state.users?.projects?.[0]?.year}</p>
       <p class="text-dark">              :{this.state.users?.projects?.[1]?.projectName} ,{this.state.users?.projects?.[1]?.year}</p>
       </div>
       <p></p>
       ...
       <p style={{color:"green"}}>{this.state.dmessage}</p>
